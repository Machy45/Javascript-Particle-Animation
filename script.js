...
//This eventlistener will take 2 attribuates, the type of event to listen for and
// call back function to run every time that event occurs.This eventlistener will
// listen for mouse movement events.Every time the user moves the mouse on the browser,
// the following code will be carried out.Event is set as a call back function by default
...
class my_particles {
  ...
  draw(){
    ctx.fillStyle = "red";
    ctx.beginPath();
    ctx.arc(this.x, this.y, this.size, 10, Math.PI * 2.2);
    ctx.closePath();
    ctx.fill();
  }
}
